...
function buildSigString(fields, passphrase = "") {
  // EXCLUDE merchant_key (and signature) from signing
...
const makePaymentId = (p="SCEND") => `${p}-${Date.now()}-${Math.random().toString(36).slice(2,8).toUpperCase()}`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({ endpoint, post: { ...postFields, signature }, debug: { SANDBOX, sigString } });
  } catch (e) {
